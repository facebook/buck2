...
impl REClientBuilder {
    ...
    pub async fn build(self) -> anyhow::Result<REClient> {
        ...
        match result.with_context(|| "The operation's result is not defined.")? {
            ...
            OpResult::Response(any) => {
                ...
                let output_files = action_result
                    ...
                    .collect::<anyhow::Result<Vec<TFile>>>()?;
                ...
                let output_directories = action_result
                    ...
                    .collect::<anyhow::Result<Vec<TDirectory2>>>()?;
        ...
        let files_with_digest: Vec<Request> = request
            ...
            .into_try_map(|x| {
                Ok::<_, anyhow::Error>(Request {
                    ...
                    data: fs_util::read_to_bytes(&x.name)?,
    ...
    pub async fn download(
        ...
        let blobs: Vec<InlinedDigestWithStatus> =
            response.into_inner().responses.into_try_map(|r| {
                Ok::<_, anyhow::Error>(InlinedDigestWithStatus {
