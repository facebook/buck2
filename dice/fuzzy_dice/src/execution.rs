...
static ALL_PANICS: Mutex<Vec<String>> = parking_lot::const_mutex(Vec::new());
            ...
            match &op {
                Operation::Query { ctx_id, var } => {
                    if let Some(ctx) = dice_ctxs.get(ctx_id) {
                        ...
                        ALL_PANICS.lock().clear();
                        // suppress print panics from dice.
                        panic::set_hook(Box::new(|info| {
                            ALL_PANICS.lock().push(
                        ...
                        if exec_result.is_ok() && !ALL_PANICS.lock().is_empty() {
...
impl Arbitrary for DiceExecutionOrder {
    fn arbitrary(g: &mut Gen) -> Self {
        ...
        // don't generate any requests out of order
        let gen_out_of_order = std::env::var("NOGEN_OUT_OF_ORDER").is_err();
