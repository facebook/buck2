...
def _native_providers(
    ...
    # explosion in the cache, this also means normal linkable behavior of also
    # adding the archives from transitive deps to the link line can cause
    # duplicate symbol errors.  So, until we fix this, don't tell
    # `create_merged_link_info` when we have `static` linkage, so that it will
    # drop propagating these deps up to the top-level link.
    # TODO(agallagher): See `create_merged_link_info` TODO.
    if preferred_linkage == Linkage("static"):
        preferred_linkage = Linkage("any")

        # Rust Python extensions do rely on static linkage though. Work around
        # this by flagging this library as an Omnibus root should it ends up
        # being used in a Python binary. Context here:
        # https://fb.workplace.com/groups/347532827186692/permalink/3097248903896494/
        providers.append(ForceConsiderationOfOmnibusRoots())
