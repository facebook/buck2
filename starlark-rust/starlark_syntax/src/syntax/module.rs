...
use crate::syntax::parser_rd::RdParser;
use crate::syntax::state::ParserState;
use crate::syntax::validate::validate_module;

/// Which parser implementation to use for parsing Starlark source.
///
/// The two implementations produce identical ASTs for any well-formed input;
/// they differ only in performance and (on errors) the phrasing of the
/// diagnostic message.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default, pagable::Pagable)]
pub enum ParserKind {
    /// LALR(1) parser generated by the [`lalrpop`](https://crates.io/crates/lalrpop)
    /// crate. The historical default; stable and well-tested.
    Lalrpop,
    /// Hand-written recursive descent + Pratt expression parser. Roughly 2x
    /// faster than [`Lalrpop`](Self::Lalrpop) on the per-parse microbenchmark.
    #[default]
    Rd,
}
...
impl AstModule {
    ...
    pub fn parse(filename: &str, content: String, dialect: &Dialect) -> crate::Result<Self> {
        Self::parse_with(filename, content, dialect, ParserKind::default())
    }

    /// Parse a Starlark module using a chosen parser implementation.
    ///
    /// Functionally equivalent to [`parse`](Self::parse) but lets the caller
    /// (e.g. buck2 driven by a buckconfig or gatekeeper) pick which parser
    /// backend to run. The two implementations produce identical ASTs for any
    /// well-formed input; they differ in performance and (on errors) the
    /// phrasing of the diagnostic.
    pub fn parse_with(
        filename: &str,
        content: String,
        dialect: &Dialect,
        ...
        // The dispatch: a single match on the runtime parser-kind value.
        let stmt = match kind {
            ParserKind::Lalrpop => LalrpopParser::parse_module(&mut state, filtered, eof_pos),
            ParserKind::Rd => RdParser::parse_module(&mut state, filtered, eof_pos),
        };
