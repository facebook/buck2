...
def apple_core_data_impl(ctx: AnalysisContext) -> ["provider"]:
...
def compile_apple_core_data(ctx: AnalysisContext, specs: [AppleCoreDataSpec.type], product_name: str) -> ["artifact", None]:
    if len(specs) == 0:
        return None
