...
apple_asset_catalog = prelude_rule(
    ...
    docs = """
        ...
        (either direct or transitive) of an [`apple_bundle()` rule](ROOT/fbsource/rule/apple_bundle.html), in which case all `apple_asset_catalog()` rules
...
apple_binary = prelude_rule(
    ...
    docs = """
        ...
        that you specify.
        See [`[].`](ROOT/fbsource/files-and-dirs/buckconfig.html#.)
        and [`[].`](ROOT/fbsource/files-and-dirs/buckconfig.html#.)
    ...
    attrs = (
        # @unsorted-dict-items
        {
            ...
            "binary": attrs.option(attrs.dep(), default = None, doc = """
                A [build target](ROOT/fbsource/concept/build_target.html) identifying
                 an [`apple_binary()` rule](ROOT/fbsource/rule/apple_binary.html) or
                 an [`apple_library()` rule](ROOT/fbsource/rule/apple_library.html) whose output will
...
apple_library = prelude_rule(
    ...
    docs = """
        ...
        source files and is similar to a [`cxx_library()` rule](ROOT/fbsource/rule/cxx_library.html) with which it shares many
...
apple_package = prelude_rule(
    ...
    docs = """
        ...
        an [`apple_bundle()` rule](ROOT/fbsource/rule/apple_bundle.html) and compresses it in
        an IPA (iOS App Store Package) file.

        This rule can be customized using the config options [`[].`](ROOT/fbsource/files-and-dirs/buckconfig.html#.)
        and [`[].`](ROOT/fbsource/files-and-dirs/buckconfig.html#.)
        .
    """,
    examples = """
    ...
    attrs = (
        # @unsorted-dict-items
        {
            "bundle": attrs.dep(doc = """
                A [build target](ROOT/fbsource/concept/build_target.html) identifying
                 an [`apple_bundle()` rule](ROOT/fbsource/rule/apple_bundle.html) whose output will
...
apple_resource = prelude_rule(
    ...
    docs = """
        ...
        an [`apple_bundle()` rule](ROOT/fbsource/rule/apple_bundle.html).
    """,
    examples = """
        ```
    ...
    attrs = (
        ...
        apple_common.info_plist_substitutions_arg() |
        {
            "test_host_app": attrs.option(attrs.dep(), default = None, doc = """
                A [build target](ROOT/fbsource/concept/build_target.html) identifying
                 an [`apple_bundle()` rule](ROOT/fbsource/rule/apple_bundle.html) that builds an
...
core_data_model = prelude_rule(
    ...
    docs = """
        ...
        (either direct or transitive) of an [`apple_bundle()` rule](ROOT/fbsource/rule/apple_bundle.html), in which case all `core_data_model()` rules
        that the bundle rule depends on are merged and placed into the final output bundle together.
    """,
    examples = """
        ```
...
prebuilt_apple_framework = prelude_rule(
    ...
    docs = """
        A `prebuilt_apple_framework()` rule represents a set of
         Objective-C/C++ source files and is very similar to a [`prebuilt_cxx_library()`](ROOT/fbsource/rule/prebuilt_cxx_library.html) rule.
    """,
    examples = """
        ```
