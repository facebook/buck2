...
# TODO(marwhal): Fix and enable on Windows
@buck_test(inplace=False, data_dir="toolchain_deps", skip_if_windows=True)
...
if watchman_dependency_linux_only():
    ...
    async def test_hash_all_commands(buck: Buck, local_only: str) -> None:
        ...
        await buck.build(
            "//:test", "-c", "test.seed=123", "-c", f"test.local_only={local_only}"
        )
        await expect_exec_count(buck, 1)

        # No longer expecting a rebuild.
        await buck.build(
            "//:test", "-c", "test.seed=456", "-c", f"test.local_only={local_only}"
        )
        await expect_exec_count(buck, 0)
...
# TODO(marwhal): Fix and enable on Windows
@buck_test(inplace=False, data_dir="projected_artifacts", skip_if_windows=True)
@pytest.mark.parametrize(
    "target",
    [
...
# TODO(marwhal): Fix and enable on Windows
@buck_test(inplace=False, data_dir="symlinks", skip_if_windows=True)
...
# TODO(marwhal): Fix and enable on Windows
@buck_test(inplace=False, data_dir="upload_all_actions", skip_if_windows=True)
