...
async def _obtain_cfg_hashes(buck: Buck) -> typing.Tuple[str, str]:
    result = await buck.cquery(
        "fbcode//buck2/tests/e2e/build/test_build_configured_data:simple",
        "--target-universe",
        "fbcode//buck2/tests/e2e/build/test_build_configured_data:universe",
    )
    [astrologer, vagabond] = result.stdout.splitlines()
    assert astrologer.startswith(
        "fbcode//buck2/tests/e2e/build/test_build_configured_data:simple (<astrologer>#"
    )
    assert vagabond.startswith(
...
async def test_build_configured_full_configuration(buck: Buck) -> None:
    ...
    result = await buck.build(
        f"fbcode//buck2/tests/e2e/build/test_build_configured_data:simple (<astrologer>#{astrologer_hash})",
        "--target-universe",
        "fbcode//buck2/tests/e2e/build/test_build_configured_data:universe",
    )
    out = (
...
async def test_build_configured_no_hash(buck: Buck) -> None:
    ...
    assert (
        f"$$$fbcode//buck2/tests/e2e/build/test_build_configured_data:simple (<vagabond>#{vagabond_hash})$$$"
        == out
    )
...
async def test_build_configured_wrong_hash(buck: Buck) -> None:
    result = await buck.build(
        "fbcode//buck2/tests/e2e/build/test_build_configured_data:simple (<vagabond>#0123456789abcdef)",
        "--target-universe",
        "fbcode//buck2/tests/e2e/build/test_build_configured_data:universe",
    )
    ...
    assert (
        "fbcode//buck2/tests/e2e/build/test_build_configured_data:simple"
        not in json.loads(result.stdout)["results"]
    )
