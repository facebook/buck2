...
def _do_not_inject_omnibus_environment_transition(
    ...
    maybe_inject_omnibus_environment = refs.maybe_inject_omnibus_environment[ConstraintSettingInfo].label
    do_not_inject_omnibus_environment = refs.do_not_inject_omnibus_environment[ConstraintValueInfo]

    constraints = dict(platform.configuration.constraints.items())
    constraints[maybe_inject_omnibus_environment] = do_not_inject_omnibus_environment

    return PlatformInfo(
        label = "<no omnibus environment>",
...
do_not_inject_omnibus_environment_transition = transition(
    impl = _do_not_inject_omnibus_environment_transition,
    refs = {
        "do_not_inject_omnibus_environment": "prelude//cxx/tools:do_not_inject_omnibus_environment",
        "maybe_inject_omnibus_environment": "prelude//cxx/tools:maybe_inject_omnibus_environment",
    },
)

def omnibus_environment_attr():
    default = select({
        "DEFAULT": "prelude//cxx/tools:omnibus_environment",
