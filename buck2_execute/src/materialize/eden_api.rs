...
pub fn is_recas_eden_mount(buck_out: &AbsNormPathBuf) -> Result<bool, SetupBuckOutError> {
...
fn execute_eden_clone(buck_out: &AbsNormPathBuf) -> Result<(), SetupBuckOutError> {
...
fn is_path_redirect(buck_out: &AbsNormPathBuf, path: &str) -> Result<bool, SetupBuckOutError> {
    let config_path = buck_out.as_path().join(".eden/client/config.toml");
    if fs::metadata(&config_path).is_err() {
        return Ok(false);
    }
...
fn setup(buck_out: &AbsNormPathBuf) -> Result<(), SetupBuckOutError> {
...
pub struct EdenBuckOut {
    ...
    re_get_session_id: Arc<dyn ReGetSessionId>,
}
...
impl EdenBuckOut {
    ...
    pub async fn set_path_object_id(
        ...
        let re_session_id = self.re_get_session_id.get_session_id().await?;
