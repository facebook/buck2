...
impl Context {
    ...
    fn go(
        ...
        let final_ast = match force_mode.unwrap_or(self.mode) {
            ContextMode::Check => {
                warnings = Either::Right(self.check(file, &ast));
                Some(ast)
            }
            ContextMode::Run => {
                let enable_print = force_mode.is_none();
    ...
    pub(crate) fn file_with_contents(
        ...
        Self::err(
            ...
            AstModule::parse(filename, content, &self.dialect)
                .map(|module| self.go(filename, module, force_mode))
                .map_err(Into::into),
        )
    }
