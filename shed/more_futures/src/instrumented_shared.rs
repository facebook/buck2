...
where
    ...
    F::Output: Clone,
{
    #[allocative(skip)] // TODO(nga): implement.
    #[pin]
    inner: Shared<F>,
    ...
    F::Output: Clone,
{
    pub fn new(fut: F) -> SharedEventsFuture<F> {
        SharedEventsFuture {
            inner: fut.shared(),
            span: SharedSpanState::Undefined,
    ...
    F::Output: Clone,
{
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        if matches!(this.span, SharedSpanState::Undefined) {
            let dispatcher = get_dispatcher();

            if *this.owner_trace_id != *dispatcher.trace_id() {
                // Send start event
                *this.span = SharedSpanState::Started(Box::new(Span::start(
                    dispatcher,
                    SharedTaskStart {
        ...
        match this.inner.poll(cx) {
            Poll::Ready(t) => {
                ...
                match std::mem::replace(this.span, SharedSpanState::Ended) {
                    SharedSpanState::Started(span) => {
                        span.end(SharedTaskEnd {});
                    }
                    SharedSpanState::Undefined | SharedSpanState::Ended => {}
                }
                Poll::Ready(t)
            }
