...
pub async fn open<P: AsRef<AbsPath>>(path: P) -> anyhow::Result<tokio::fs::File> {
...
pub async fn read_to_string<P: AsRef<AbsPath>>(path: P) -> anyhow::Result<String> {
    let path = path.as_ref().to_owned();
...
pub async fn create_dir_all<P: AsRef<AbsPath>>(dir: P) -> anyhow::Result<()> {
