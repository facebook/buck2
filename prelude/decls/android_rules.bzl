...
android_library(), android_resource(), cxx_library(), groovy_library(), java_library(), java_binary(), prebuilt_jar(), prebuilt_native_library().""",
        ...
        "include_build_info_file": attrs.bool(
            default = read_root_config("android", "include_build_info_file", "false").lower() == "true",
            doc = "Bake `assets/BuildInfo.json` with the buck2 `BUCK_BUILD_ID` into the APK so an installed build can be resolved back to its buck2 build and source commit. On by default for non-release builds (`[android] include_build_info_file` in `fbsource-common.bcfg`); pass `-c android.include_build_info_file=false` to disable. The `apk_build` action invokes a toolchain-provided generator to write the file, so the baked id refreshes when the APK content changes and a no-op rebuild reuses a prior (equally valid) id.",
        ),
