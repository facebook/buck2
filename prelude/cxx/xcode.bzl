...
def _get_artifacts_with_owners(files: "") -> {"artifact": {str.type: str.type}}:
    if type(files) == "dict":
...
def _get_artifact_owner(file: "artifact") -> {str.type: str.type}:
    if file.owner:
        return {"target": str(file.owner.raw_target())}
    else:
        return {}
