...
def _materialize_template(template: str, args: argparse.Namespace) -> str:
    ...
    if args.target_platform != "linux":
        ...
        # On linux, use the real python interpreter but run through `which` first to resolve
        python_replacement = shutil.which(args.python)
        new_data = data.replace("<PYTHON>", python_replacement)
