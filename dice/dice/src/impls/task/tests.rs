...
async fn simple_task() -> anyhow::Result<()> {
    ...
    let (task, _handle) = spawn_dice_task(DiceKey { index: 10 }, &TokioSpawner, &(), |handle| {
        async move {
...
async fn not_ready_until_dropped() -> anyhow::Result<()> {
    ...
    let (task, _handle) = spawn_dice_task(DiceKey { index: 500 }, &TokioSpawner, &(), |handle| {
...
async fn never_ready_results_in_terminated() -> anyhow::Result<()> {
    let (task, handle) = spawn_dice_task(DiceKey { index: 500 }, &TokioSpawner, &(), |handle| {
        async move {
            let _handle = handle;
            // never report ready
        }
...
async fn multiple_promises_all_completes() -> anyhow::Result<()> {
    let (task, handle) = spawn_dice_task(DiceKey { index: 20 }, &TokioSpawner, &(), |handle| {
        async move {
            // wait for the lock too
            handle.finished(DiceComputedValue::new(
                MaybeValidDiceValue::valid(DiceValidValue::testing_new(DiceKeyValue::<K>::new(2))),
...
async fn sync_complete_task_completes_promises() -> anyhow::Result<()> {
    let (task, waiting_handle) = sync_dice_task(DiceKey { index: 100 });
...
async fn sync_complete_task_with_future() -> anyhow::Result<()> {
    let (task, _waiting_handle) = sync_dice_task(DiceKey { index: 100 });
...
async fn sync_complete_unfinished_spawned_task() -> anyhow::Result<()> {
    ...
    let (task, _handle) = spawn_dice_task(DiceKey { index: 88 }, &TokioSpawner, &(), {
        let lock = lock.dupe();
        |handle| {
            async move {
...
async fn sync_complete_finished_spawned_task() -> anyhow::Result<()> {
    ...
    let (task, _handle) = spawn_dice_task(DiceKey { index: 44 }, &TokioSpawner, &(), {
        let sem = sem.dupe();
        |handle| {
            async move {
                // wait for the lock too
...
async fn dropping_all_waiters_cancels_task() {
    ...
    let (task, handle) = spawn_dice_task(DiceKey { index: 600 }, &TokioSpawner, &(), {
        let barrier = barrier.dupe();
        |handle| {
            async move {
                let _handle = handle;
...
async fn task_that_already_cancelled_returns_cancelled() {
    let (task, handle) = spawn_dice_task(DiceKey { index: 777 }, &TokioSpawner, &(), {
...
async fn dropping_termination_observer_does_not_cancel_task() {
    ...
    let (task, _handle) = spawn_dice_task(DiceKey { index: 888 }, &TokioSpawner, &(), {
        let barrier = barrier.dupe();
        |handle| {
            async move {
                let _handle = handle;
