...
async def test_external_buckconfigs(buck: Buck) -> None:
    ...
    if sys.platform == "darwin":
        ...
        injected_config = external_configs[i]["data"]["ConfigValue"]
        assert (
            injected_config["section"] == "buck2"
            and injected_config["key"] == "file_watcher"
            and injected_config["value"] == "fs_hash_crawler"
        )
        i += 1
    assert len(external_configs) == 4 + i

    # The order is important here. We first have the buckconfig values from external sources
    external_path_config_value = external_configs[i]["data"]["ConfigValue"]
    assert (
    ...
    assert not external_path_config_value["is_cli"]
    ...
    assert (
        external_configs[i + 1]["data"]["ConfigFile"]["data"]["ProjectRelativePath"]
        == "my_mode.bcfg"
    )

    # Then comes the config flag from the cli
    config_flag = external_configs[i + 2]["data"]["ConfigValue"]
    ...
    config_file = external_configs[i + 3]["data"]["ConfigFile"]["data"][
