...
/// builder never leaves this type unsealed: [`seal`](ModuleHeaps::seal) and [`Drop`] are its only
/// exits, and both seal it into the value heap's references. The edge therefore exists from the
/// moment the two heaps do, with nothing to remember at the end.
#[derive(Debug)]
pub(crate) struct ModuleHeaps<'v> {
    heap: Heap<'v>,
    /// `None` only after `seal` has taken the builder out; `Drop` then has nothing to do.
...
impl<'v> ModuleHeaps<'v> {
    ...
    pub(crate) fn frozen_heap<R>(
        ...
        self.frozen().with(|fh| {
            ...
            // through `seal` or `Drop`, both of which seal it into `heap`'s references. So
    ...
    fn frozen(&self) -> &OwnedFrozenHeap {
        self.frozen
            .as_ref()
            .expect("the builder is only taken by `seal`, which consumes `self`")
    }

    /// Seal the frozen heap into the value heap's references and return it.
    ///
    /// The `name` is the sealed heap's, see [`OwnedFrozen::name`].
    pub(crate) fn seal(mut self, name: Option<FrozenHeapName>) -> OwnedFrozen<()> {
        let frozen = self
            .frozen
            .take()
            .expect("the builder is only taken by `seal`, which consumes `self`");
        self.seal_into_heap(frozen, name)
    }

    fn seal_into_heap(
        &self,
        frozen: OwnedFrozenHeap,
...
impl<'v> Drop for ModuleHeaps<'v> {
    fn drop(&mut self) {
        ...
        if let Some(frozen) = self.frozen.take()
            && !frozen.is_empty()
        {
            self.seal_into_heap(frozen, None);
        }
    }
}
