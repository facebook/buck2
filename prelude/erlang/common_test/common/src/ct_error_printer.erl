...
-spec format_error
    (ErrorType, Reason, FormatStacktrace :: false) -> unicode:chardata() when
        ErrorType :: error | exit | throw,
        Reason :: term();
    (ErrorType, {Reason, StackTrace}, FormatStackTrace :: true) -> unicode:chardata() when
        ErrorType :: error | exit | throw,
        Reason :: term(),
...
-define(FORMAT_ASSERTION(Format, Expressions, SpecificExpressions),
    {ok, [
        ...
        SpecificExpressions,
        print_comments(
            Props, string:chr(unicode_characters_to_string(lists:nth(1, SpecificExpressions)), $:)
        )
    ]}
).
...
print_type_expression(Type, Format, Args) ->
    case unicode:characters_to_binary(io_lib:format("Failure ?~tp(~ts)", [Type, Format])) of
        FormatStr when is_binary(FormatStr) ->
