...
def _find_all_relevant_roots(
    ...
    for spec in specs:
        if spec.root != None:
            ...
            relevant_roots[spec.group.name] = roots
            if has_empty_root:
                link_groups_for_full_traversal.add(spec.group.name)

    for node_target, node_link_group in link_group_mappings.items():
        ...
        if node.preferred_linkage == Linkage("static") and not node.ignore_force_static_follows_dependents:
            # Node should be linked statically into final binary
            continue

        if node_link_group == MATCH_ALL_LABEL:
            # Add link group into the list of roots for all link groups
...
def create_link_groups(
    ...
    undefined_symfiles = []
    global_symfiles = []
    roots = _find_all_relevant_roots(specs, link_group_mappings, linkable_graph_node_map)
