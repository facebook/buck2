...
Note that this change does not apply to test execution. See [Test Migration][#test-migration] below for details.
...
1. Commands are run in a sandbox with no access to undeclared dependencies, network access or locally installed tools. These restrictions increase cachability and reproducibility, so are strongly encouraged. Where meeting these restrictions is not feasible (e.g. use of `hg`), rules can be opted into local execution by adding `labels = ["my_reason_for_running_locally"]` to the `genrule` and adding the exception to [genrule_local_labels.bzl](https://www.internalfb.com/code/fbsource/fbcode/buck2/prelude/genrule_local_labels.bzl) under `_LOCAL_LABELS`.
...
* Rules relying on tools that are not deployed on RE, e.g. `jar`, can be executed remotely using an `$(exe)` reference to the tool in the repo, e.g. `fbsource//third-party/toolchains`. See [D40818900](https://www.internalfb.com/diff/D40818900).
...
1. The `export_file` rule in Buck v1 sometimes uses the basename of the exported files and sometimes the short name -- it depends on how the exported file is consumed. For example, exporting `foo/bar.txt` will give the name `bar.txt` in a Buck1 `genrule` and `foo/bar.txt` in Buck2. There are three solutions: 1) use `export_file` on a `src` with no slashes; 2) use `mode = "copy"` in `export_file` and an explicit `out` attribute; 3) for `genrule` use an explicit `srcs` dictionary, e.g. change `srcs = [":foo"]` to `srcs = {"foo": ":foo"}`.
2. In Buck v1, if you are using user providers, a value not present is considered missing (`hasattr` returns `False`), whereas in Buck2 that field is set to `None` (`hasattr` returns `True`).
3. There are no "flavors" in Buck2, where a flavor is a target modified by appending `#something`. For example, Rust binaries could be checked (not compiled, just type checked) with `library#check`, whereas in Buck2 that is `library[check]` (using the named targets syntax). Where libraries are depended upon using flavours for stripping or platform transition those should use [configuration mechanisms](rule_authors/configurations.md) instead.
4. The C++ link order in Buck v1 and Buck2 is unspecified, but for some C++ projects there are duplicate symbols defined, where v1 happens to pick a link order that works but v2 doesn't. In such cases the duplicate symbols should be fixed properly so either link order works, usually by renaming one symbol.
5. When run on Remote Execution, input files will be hard links, and identical inputs will link to the same underlying file. Most tools are oblivious to hard links, but some such as `tar` may require flags such as `--hard-dereference`, and `cp` may require `--dereference`.

### Historical notes

These are issues that we think we have fixed throughout the repo, so should no longer be encountered.

1. Targets files must use Starlark, not Python - e.g. no `f"strings"`, float, globals etc. In particular `type(x)` in Starlark returns a string, so `type(x) == type("")` is the recommended compatible way to work with Starlark and Python.
2. We are more strict about parsing macros, e.g. `$(location target` without a closing bracket used to be accepted, but isn't anymore.
3. If you depend on a source file with a `../` prefix it will be an error.
...
1. C++ files are compiled relative to the project root (e.g. `fbsource`) rather than the cell root (e.g. `fbcode`). As a consequence, the `__FILE__` preprocessor macro will have a leading `fbcode/` prefix when building with Buck2.
2. While not required, we encourage greater use of `buck2 run`. With `buck` the `run` command could be quite slow, so people often bypassed it, by compiling and then executing directly from `buck-out`. With `buck2` the `run` command is fast (often milliseconds) so is encouraged.
3. By default Buck2 only downloads the files that are direct runtime dependencies of the terminal target, so if your binary invokes another binary or loads files, these should be declared as runtime dependencies (see the next section).
4. In Buck2 all flag arguments after either `--` or the second positional argument are passed to the target, while with Buck1 it is only after `--`, e.g. `buck run MY_TARGET foo -x` will pass `-x` to Buck itself with Buck1, but to target with Buck2. As another example, `buck2 run MY_TARGET foo -- bar` will pass `foo bar` to Buck1, but `foo -- bar` to Buck2. To ensure compatibility, put the `--` after the target, to give `buck2 run MY_TARGET -- foo bar` which will produce `foo bar` in both Buck1 and Buck2.
...
1. With the `resources` attribute you can specify files so they can be retrieved at runtime with language-specific APIs. Resources are supported in a few languages including [C++](https://www.internalfb.com/intern/wiki/Buck-users/C++_Resources/) (e.g. using [`build::getResourcePath`](https://www.internalfb.com/code/search?q=repo%3Afbcode%20build%3A%3AgetResourcePath)) and [Python](https://www.internalfb.com/intern/wiki/Buck-users/Python_Resources_in_fbcode/) (as shown in [D27072737](https://www.internalfb.com/diff/D27072737)). If you wish to depend on an executable, including all its shared objects, etc., this mechanism may not be powerful enough.
   * In Buck1 the name of each resource defaults to the target that provides the resource, while in Buck2 it defaults to the file produced by the target. To get around this, you can either adjust the target name so it matches the output name, or specify a mapping to name the resources explicitly. To explicitly name resources for Python rules, set resources to `{":target", "name"}`. For C++, use the pattern `{"name", ":target"}`.
   * If you are working in arvr-related directories, the attribute you want here might be named `cxx_resources`.
2. Using `command_alias` you can declare a runtime dependency in the `args` or `env` attributes, using either `$(location ...)` or `$(exe_target ...)`, which will ensure the files are available at runtime and pass them to the command.
...
* The path to files when a build is ongoing is also relative to the repository root. This can impact e.g. log paths (since your file paths will now include the `fbcode` prefix). However, note that the working directory when executing tests didn't change, see the next section for details.
...
### Different working directory between rule execution and tests

The [change to working directories][#changed-working-directory] only applies to rule execution, **not to test execution**. This discrepancy exists to facilitate migration from Buck1 to Buck2: tests that depend on their working directory are a lot more common than rules.

We generally recommend making your tests not depend on undeclared inputs, just like rules (e.g. by adding `resources` if you need to expose test fixtures). That being said, if this discrepancy is creating issues for your tests and updating your code is not an option, we allow users to opt in to run their tests from the project root like rules for supported test types:

+ For C++ tests and `sh_test`, you can opt in by adding the `buck2_run_from_project_root` label to your rule's labels. Note that in the fbcode macros (`cpp_unittest`, `custom_test`), labels are set by passing the `tags` argument.

If you need this mechanism in another test type, ask in [Buck2 users](https://fb.prod.workplace.com/groups/buck2users).

## Workflow migration

1. In Buck2 all output files are relative to the project root (e.g. `fbsource/buck-out`) rather than the cell root as in Buck1 (e.g. `fbsource/fbcode/buck-out`).
2. In Buck2 all output paths include a hash of the current configuration, for example `buck-out/v2/gen/fbcode/8107dfcc75db0fad/foo/libbar.so` contains the hash `8107dfcc75db0fad`. Such files can _also_ be found at `buck-out/v2/gen/fbcode/foo/libbar.so` (without the hash), due to a compatibilty option that is enabled. However, we would encourage using `buck2 run` or `buck2 build --show-output` to obtain the location of the files rather than hardcoding their location, as their unhashed location will change in future.
3. In Buck2 building a library target (e.g. one defined with `cpp_library`) causes the target to be compiled, which is not the case for Buck v1 (i.e. Buck v1 will not notice syntax errors or such). That can cause differences when doing `buck2 build fbcode//myproject:my_library_target`, or when doing patterns that include `my_library_target` such as `buck2 build fbcode//...` or `buck2 build fbcode//myproject:`.
...
* Conveyor Hotfix corner case : Hotfix feature doesnt support 2 contbuilds. Thus if you would like to use 2 fbpkgs(v1, v2) in conveyor you need to build those in a single contbuild. example: [D37082297](https://www.internalfb.com/diff/D37082297) .
```python
"buck2_overrides": {
      "owners": ["<username>"],
      "run_tests": True,
...
The Buck version used by `fbpkg.builder` is configuerd on the build rule itself. By default it is using Buck2. To switch it to Buck1 use `version="v1"` parameter. (examples: [D35663292](https://www.internalfb.com/diff/D35663292))
...
You might need to change the rules by removing prefix "//" (example: [D34769694](https://www.internalfb.com/diff/D34769694)).
...
your par directives into full rules. See [D34559280](https://www.internalfb.com/diff/D34559280) for an example.
