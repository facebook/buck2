...
struct Args {
    #[structopt(long = "--buck-command", default_value = "buck2")]
    buck_command: String,
    #[structopt(long = "--destination-dir")]
    destination_dir: PathBuf,
    #[structopt(long = "--native-subdir", default_value="native", parse(try_from_str = PathBuf::try_from))]
    native_subdir: PathBuf,
    #[structopt(long = "--starlark-subdir", default_value="starlark", parse(try_from_str = PathBuf::try_from))]
    ...
    #[structopt(
        ...
        help = "Patterns to interpret. //foo:bar.bzl is 'every symbol in //foo:bar.bzl'"
    )]
    symbol_patterns: Vec<String>,
}

fn get_docs_json(args: &Args) -> anyhow::Result<String> {
    let mut buck2_args = vec!["docs".to_owned(), "starlark".to_owned()];
    if args.builtins {
        buck2_args.push("--builtins".to_owned());
    }
    buck2_args.extend(args.symbol_patterns.clone().into_iter());
    eprintln!("Invoking `{} {}`", args.buck_command, buck2_args.join(" "));
    let cmd = Command::new(&args.buck_command)
        .args(buck2_args)
        .stderr(Stdio::inherit())
        .output()?;
    if !cmd.status.success() {
        Err(anyhow::anyhow!(
            "`{} docs` failed with status code {}",
            args.buck_command,
...
fn command_for_generated_annotation(command_for_display: Vec<String>) -> String {
    let mut cmd = vec![];
    for arg in command_for_display.into_iter().skip(1) {
        // Do this logic so that if someone runs with `./buck2.sh`, or `buck2`, with no changes,
        // they won't regenerate their .md files.
        let arg = if arg.starts_with("--buck-command=") {
            "--buck-command=buck2".to_owned()
        } else {
            arg
        };
        match cmd.last() {
            Some(a) if a == "--buck-command" => cmd.push("buck2".to_owned()),
            _ => cmd.push(arg),
        }
    }
    format!("buck2_docs {}", cmd.join(" "))
}

/// Does the heavy work of processing the docs and writing them to markdown files.
///
/// Separated from main() mostly to help with testing.
fn process_docs(
    command_for_display: Vec<String>,
    args: Args,
    buck2_docs_invoker: &dyn Fn(&Args) -> anyhow::Result<String>,
) -> anyhow::Result<()> {
    let json = buck2_docs_invoker(&args)?;
    let docs: Vec<Doc> = serde_json::from_str(&json)?;

    let mut outputs = HashMap::new();

    for doc in docs.into_iter().sorted_by(|l, r| l.id.name.cmp(&r.id.name)) {
        let markdown_path = MarkdownOutput::markdown_path_for_doc(&args, &doc)?;
    ...
    let abs_destination = if args.destination_dir.is_relative() {
        std::env::current_dir()?.join(args.destination_dir)
    } else {
        args.destination_dir
    };

    let cmd = command_for_generated_annotation(command_for_display);
    let preamble = format!(
    ...
    for (relative_path, markdown_file) in outputs.iter() {
        ...
        eprintln!("Writing to {}", path.to_str().unwrap());
        markdown_file.write_to_file(&path, &preamble)?;
    }
...
"####;
    ...
    fn write_to_file(&self, path: &Path, preamble: &str) -> anyhow::Result<String> {
        let mut contents = format!("{}{}", preamble, self.markdown_contents());
    ...
    fn markdown_path_for_doc(args: &Args, doc: &Doc) -> anyhow::Result<PathBuf> {
        let path = match &doc.id.location {
            ...
            None => match &doc.item {
                ...
                // Artifact, etc)
                DocItem::Module(_) | DocItem::Function(_) => args.native_subdir.join("native"),
                DocItem::Object(_) => args.native_subdir.join(&doc.id.name),
            },
        };
...
fn main() -> anyhow::Result<()> {
    let raw_command = std::env::args().collect();
    let args = Args::from_args();
    process_docs(raw_command, args, &get_docs_json)
}
...
mod tests {
    ...
    fn test_generates_correct_markdown() -> anyhow::Result<()> {
        ...
        let json_string = serde_json::to_string(&sample_data)?;
        ...
        let raw_command = vec![
            ...
            "starlark".to_owned(),
            "--builtins".to_owned(),
            "foo//bar:baz.bzl".to_owned(),
        ];
        let args = Args {
            buck_command: "buck2".to_owned(),
            destination_dir: temp.path().to_path_buf(),
            native_subdir: PathBuf::try_from("native")?,
            starlark_subdir: PathBuf::try_from("starlark")?,
            builtins: true,
            symbol_patterns: vec!["foo//bar:baz.bzl".to_owned()],
        };

        process_docs(raw_command, args, &|_| Ok(json_string.clone()))?;
