...
impl LegacyBuckConfigView for LegacyBuckConfig {
    fn get(&self, section: &str, key: &str) -> Option<Arc<str>> {
        self.get(section, key).map(|v| v.to_owned().into())
    }
}
...
pub trait ConfigParserFileOps {
    ...
    fn file_exists(&self, path: &AbsPath) -> bool;

    fn file_id(&self, path: &AbsPath) -> String {
        path.to_string()
    }
}
...
impl<'a> LegacyConfigParser<'a> {
    ...
    fn push_file(&mut self, line: usize, path: &AbsPath) -> anyhow::Result<()> {
    ...
    fn start_file(&mut self, path: &AbsPath, source: Option<Location>) -> anyhow::Result<()> {
        let source_file = Arc::new(ConfigFile {
            id: self.file_ops.file_id(path),
    ...
    fn parse_file_on_stack(&mut self, path: &AbsPath, parse_includes: bool) -> anyhow::Result<()> {
        let parent = path
            .parent()
...
impl LegacyBuckConfig {
    ...
    fn resolve_config_flag_arg(
        ...
        let cell_path: Option<AbsPathBuf>;
        let raw_config: &str;
    ...
    fn resolve_config_file_arg(
        ...
        AbsPathBuf::try_from(file_arg.to_owned())
    }
                ...
                match buckconfig {
                    ...
                    BuckConfigFile::UserFile(file) => {
                        ...
                        if let Some(home_dir_path) = home_dir {
                            let buckconfig_path = ForwardRelativePath::new(file)?;
                            buckconfig_paths.push(MainConfigFile {
                                path: AbsPath::new(&home_dir_path)?
                    ...
                    BuckConfigFile::UserFolder(folder) => {
                        ...
                        if let Some(home_dir_path) = home_dir {
                            ...
                            let buckconfig_folder_abs_path =
                                AbsPath::new(&home_dir_path)?.join_normalized(buckconfig_path)?;
                            push_all_files_from_a_directory(
                                &mut buckconfig_paths,
                                &buckconfig_folder_abs_path,
                                false,
                    ...
                    BuckConfigFile::GlobalFile(file) => {
                        buckconfig_paths.push(MainConfigFile {
                            path: AbsPathBuf::from(String::from(*file))?,
                            owned_by_project: false,
                        });
                    }
                    BuckConfigFile::GlobalFolder(folder) => {
                        let buckconfig_folder_abs_path = AbsPathBuf::from(String::from(*folder))?;
                        push_all_files_from_a_directory(
                            &mut buckconfig_paths,
                            &buckconfig_folder_abs_path,
                            false,
...
fn push_all_files_from_a_directory(
    ...
    for entry in readdir {
        ...
        if entry_path.is_file() {
            buckconfig_paths.push(MainConfigFile {
                path: AbsPath::new(&entry_path)?.to_buf(),
                owned_by_project,
            });
        } else {
            tracing::warn!(
...
pub mod testing {
    ...
    pub struct TestConfigParserFileOps {
        data: HashMap<AbsPathBuf, String>,
    }
