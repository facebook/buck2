    ...
    impl Key for ConfiguredGraphKey {
        ...
        async fn compute(&self, ctx: &DiceComputations) -> Self::Value {
            ...
            let deps = keep_going::try_join_all(
                node.deps()
                    .map(|label| get_configuration_graph_node(ctx, label.target()))
...
async fn get_analysis_result(
    ...
    let mut dep_analysis = keep_going::try_join_all(
        configured_node
            ...
            .map(async move |dep| {
                let res = ctx
                    .get_analysis_result(dep.target())
                    .await
            ...
            .eval_import(
                ...
                indoc!(r#"
                            ...
                            def impl(ctx):
                                str = ""
                                if ctx.attr.dep:
                                    str = ctx.attr.dep[FooInfo].str
                                return [FooInfo(str=(str + ctx.attr.str)), DefaultInfo()]
                            foo_binary = rule(implementation=impl, attrs={"dep": attr.option(attr.dep(providers=[FooInfo])), "str": attr.string()})
                        "#),
