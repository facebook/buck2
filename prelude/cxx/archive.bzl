...
def _archive(cxx_context: CxxContext.type, name: str.type, args: "cmd_args", thin: bool.type, prefer_local: bool.type) -> "artifact":
    archive_output = cxx_context.actions.declare_output(name)
    ...
    cxx_context.actions.run(command, category = category, identifier = name, prefer_local = prefer_local)
...
def make_archive(
    ...
    archive = _archive(cxx_context, name, args, thin = thin, prefer_local = linker_info.archive_objects_locally)
