...
impl ForwardRelativePath {
    ...
    ///
    /// use std::path::Path;
    /// use buck2_core::fs::paths::{ForwardRelativePath, ForwardRelativePathBuf, AbsPath, AbsPathBuf};
    ///
    /// if cfg!(not(windows)) {
    ///     let path = ForwardRelativePath::new("foo/bar")?.resolve(AbsPath::new("/some")?);
    ///     assert_eq!(AbsPathBuf::from("/some/foo/bar".into())?, path);
    /// } else {
    ///     let path = ForwardRelativePath::new("foo/bar")?.resolve(AbsPath::new("c:/some")?);
    ///     assert_eq!(AbsPathBuf::from("c:/some/foo/bar".into())?, path);
    /// }
    ///
    /// # anyhow::Ok(())
    /// ```
    ...
    ///
    /// ```
    /// use buck2_core::fs::paths::{ForwardRelativePath, FileName};
    ///
...
impl ForwardRelativePathBuf {
    ...
    /// ```
    ///
    /// use buck2_core::fs::paths::{RelativePath, ForwardRelativePathBuf};
    ///
    /// let mut path = ForwardRelativePathBuf::unchecked_new("foo".to_owned());
