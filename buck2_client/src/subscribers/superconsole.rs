...
impl StatefulSuperConsole {
    ...
    pub fn new_with_root_forced(
        ...
        Self::new(
            builder.build_forced(root, default_size)?,
            verbosity,
    ...
    pub fn render_console(self) -> anyhow::Result<()> {
        if let Some(mut sc) = self.super_console {
            sc.render(&self.state.state())?;
            return sc.output.finalize();
        }
        Err(anyhow::anyhow!("Cannot render non-existent superconsole"))
    }
}
