...
load(":apple_utility.bzl", "get_apple_cxx_headers_layout", "get_apple_frameworks_linker_flags", "get_framework_search_path_flags", "get_module_name")
...
AppleLibraryAdditionalParams = record(
    ...
    extra_link_flags = field(["_arglike"], []),
...
def apple_library_rule_constructor_params_and_swift_providers(ctx: "context", params: AppleLibraryAdditionalParams.type) -> (CxxRuleConstructorParams.type, ["provider"]):
    extra_exported_link_flags = flatten(get_apple_frameworks_linker_flags(ctx)) + get_min_deployment_version_target_linker_flags(ctx) + params.extra_link_flags
