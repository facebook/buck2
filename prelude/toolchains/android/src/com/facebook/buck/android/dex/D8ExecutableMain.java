...
public class D8ExecutableMain {
  ...
  private void run() throws IOException {
    ...
    try {
      ...
      if (refCountOutput != null) {
        ...
        // For pre-dex (intermediate) mode, the output is a .dex.jar containing one or more
        // dex files (classes.dex, classes2.dex, ...). Large libraries may produce multi-dex
        // output, so we sum ref counts across all dex files in the jar.
  ...
  /**
   * Sums method_ids_size, field_ids_size, and type_ids_size across all .dex entries in a jar. Large
   * libraries may produce multi-dex output (classes.dex, classes2.dex, ...); all dex files must be
   * counted. Returns zeros if the jar contains no .dex entries (resource-only AARs).
   */
