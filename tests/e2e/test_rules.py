...
if _is_running_on_linux():
    ...
    async def test_go(buck: Buck) -> None:
        await buck.build("//buck2/tests/targets/rules/go/...")

    @buck_test(inplace=True)
    async def test_go_tags(buck: Buck) -> None:
        await buck.build(
            "//buck2/tests/targets/rules/go/tags:example-check",
            "-c",
            "go.tags=tag1,tag2",
        )
        await expect_failure(
            buck.build("//buck2/tests/targets/rules/go/tags:example-check"),
