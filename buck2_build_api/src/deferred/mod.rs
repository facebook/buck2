...
#[derive(Clone_, Dupe, Display, Derivative)]
#[derivative(Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
#[display(fmt = "{}", key)]
...
#[derive(Clone, Dupe, Display, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
...
impl DeferredRegistry {
        ...
        F: Fn(&T, &mut dyn DeferredCtx) -> DeferredValue<U> + Send + Sync + 'static,
    {
        ...
        where
            T: Send + Sync + 'static,
            F: Fn(&T, &mut dyn DeferredCtx) -> DeferredValue<U>,
        {
            type Output = U;
...
impl DeferredValueAny {
    fn ready<T: Clone + Debug + Send + Sync + 'static>(t: T) -> Self {
...
/// untyped deferred
pub trait DeferredAny: Send + Sync {
    /// the set of 'Deferred's that should be computed first before executing this 'Deferred'
    fn inputs(&self) -> &IndexSet<DeferredInput>;
...
#[derive(Clone, Copy, Debug, Dupe, Display)]
...
mod tests {
    ...
    impl<T: Clone + Debug + Send + Sync + 'static> Deferred for DeferringDeferred<T> {
        type Output = T;
    ...
    fn make_resolved<T: Clone + Debug + Send + Sync + 'static>(
        data: &DeferredData<T>,
