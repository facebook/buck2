...
fn is_python_module(file: &str) -> bool {
    const SOURCE_EXTENSIONS: [&str; 4] = ["py", "so", "empty_stub", "pyd"];
    const IGNORE_PATHS: [&str; 1] = ["runtime/lib"];

    if IGNORE_PATHS.iter().any(|&ignore| file.starts_with(ignore)) {
        return false;
    }
    SOURCE_EXTENSIONS.iter().any(|&ext| file.ends_with(ext))
}

fn process_file(filepath: &str, module_map: &mut HashMap<String, bool>) {
    if !is_python_module(filepath) {
        return;
    }
...
fn main() -> anyhow::Result<()> {
    ...
    if let Some(resources) = &args.resources {
        symlink_files(resources, &args.output_path)?;
    }
